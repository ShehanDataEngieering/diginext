...
import type { Item, ItemUnitWithDetails, PhotoLogEntry, Project } from '../../shared/ipc'
...
export async function buildProjectInventoryWorkbook(
  ...
  const newToolboxSheetName = `toolbox ${newSheetName}`
  ...
  if (existingWorkbook) {
    for (const oldSheet of existingWorkbook.worksheets) {
      ...
      if (oldSheet.name === newToolboxSheetName) continue
...
// --- Toolbox photos sheet (project photo log) ------------------------------

async function buildToolboxPhotosSheet(
  workbook: ExcelJS.Workbook,
  photoLog: PhotoLogEntry[],
  projectId: number,
  dateSheetName: string
): Promise<void> {
  const entries = photoLog.filter((e) => e.projectId === projectId)
  if (entries.length === 0) return

  const sheetName = `toolbox ${dateSheetName}`
  const sheet = workbook.addWorksheet(sheetName)
